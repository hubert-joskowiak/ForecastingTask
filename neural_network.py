...
def dnn(train_features, train_labels, test_features):
    ...
    with tf.Session() as sess:
        ...
        for i in range (num_iters):
                ...
                if i%(iters_in_epoch*100) == 0:
                    print('[{}] cost: {}'.format(epoch, np.mean(CT)))

                
                if i%iters_in_epoch == 0:
                    cost_list.append(np.mean(CT))
                    epoch += 1
                    CT = []
        ...
        predictions = sess.run([Y], {X : batch_X, Y_hat : batch_Y})
